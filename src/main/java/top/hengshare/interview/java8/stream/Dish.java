...
public class Dish {
	...
	public static void main(String[] args) {
		List<Dish> menu = Arrays
				.asList(new Dish("pork", false, 800, Dish.Type.MEAT), new Dish("beef", false, 700, Dish.Type.MEAT),
		...
		//跳过元素
		menu.stream().filter(d -> d.getCalories() > 300).skip(3).collect(toList()).forEach(System.out::println);
		...
		someNumbers.stream().map(x -> x * x).filter(x -> x % 3 == 0).findFirst().ifPresent(System.out::println); // 9
